...
describe('MCP schemas surface new options', () => {
  ...
  it('registers nexus_stats with session + recent_limit', async () => {
    const tools = await getRegisteredTools();
    const tool = tools.find(t => t.name === 'nexus_stats');
    expect(tool).toBeDefined();
    const schema = tool!.inputSchema as { properties?: Record<string, unknown> };
    expect(schema.properties).toHaveProperty('session');
    expect(schema.properties).toHaveProperty('recent_limit');
  });
